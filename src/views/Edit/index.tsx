...
export default function Edit(props: Any) {
  ...
  const handleDone = async (...args: Any[]) => {
    ...
    const save = async (...args: Any[]) => {
      ...
      const [global, inputs, outputs] = M.createInputsOutputs(
        sources,
        profiles,
        true,
      );
      ...
      if (err !== null) {
        notify.Dispatch(
          "error",
          "save:ingest",
          i18n._(t`Failed to update ingest process (${err.message})`),
        );
        return false;
      }
      ...
      if (res === false) {
        notify.Dispatch(
          "warning",
          "save:ingest",
          i18n._(t`Failed to save ingest metadata`),
        );
      }
      ...
      if (err !== null) {
        notify.Dispatch(
          "error",
          "save:ingest",
          i18n._(t`Failed to update ingest snapshot process (${err.message})`),
        );
      }
      ...
      if (res === false) {
        notify.Dispatch(
          "warning",
          "save:ingest",
          i18n._(t`Failed to update the player`),
        );
      }
      ...
      if (res === false) {
        notify.Dispatch(
          "warning",
          "save:ingest",
          i18n._(t`Failed to update the playersite`),
        );
      }
  ...
  const handleChannelDelete = async (...args: Any[]) => {
    ...
    if (res === false) {
      ...
      notify.Dispatch(
        "warning",
        "delete:ingest",
        i18n._(t`The channel "${$data.meta.name}" could not be deleted`),
      );
      return;
    }
  ...
  return (
    <Root>
        ...
        <Grid container spacing={1}>
          <TabsVerticalGrid>
            ...
            >
              ...
              <Tab
                className="tab"
                label={<Trans>Processing &amp; Control</Trans>}
                value="control"
              />
              <Tab
                className="tab"
                label={<Trans>Meta information</Trans>}
                value="meta"
              />
              <Tab
                className="tab"
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={12}>
                  <Typography variant="body1">
                    <Trans>
                      Edit the audio and video sources for the live stream. Add
                      a description, and set your desired content license.
                    </Trans>
                  </Typography>
                </Grid>
                {$state.editing === false && (
                  <Grid size={12}>
                    <div className={classes.wizardButtonElement}>
                      <Typography>
                        <Trans>
                          Use the wizard (
                          <AutoFixHighIcon className={classes.inlineIcon} />
                          ) for a quick and easy setup, or edit (
                          <EditIcon className={classes.inlineIcon} />) the
                          sources directly in custom mode.
                        </Trans>
                      </Typography>
                    </div>
                  </Grid>
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={12}>
                  <SnapshotControl
                    ...
                    onChange={handleControlChange("snapshot")}
                  />
                </Grid>
                <Grid size={12}>
                  <Divider />
            ...
            <TabPanel value={$tab} index="meta" className="panel">
              <Grid container spacing={2}>
                ...
                <Grid size={12}>
                  <Typography>
                    <Trans>
                      Briefly describe what the audience will see during the
                      live stream.
                    </Trans>
                  </Typography>
                </Grid>
            ...
            <TabPanel value={$tab} index="license" className="panel">
              <Grid container spacing={2}>
                ...
                <Grid size={12}>
                  <Typography>
                    <Trans>
                      Use your copyright and choose the right image licence.
                      Whether free for all or highly restricted. Briefly discuss
          ...
          buttonsRight={
            <React.Fragment>
              ...
              <Button
                variant="outlined"
                color="secondary"
                onClick={handleChannelDeleteDialog}
              >
                <Trans>Delete</Trans>
              </Button>
            </React.Fragment>
      ...
      <Dialog
        ...
        buttonsLeft={
          <Button
            variant="outlined"
            color="default"
            onClick={handleSourceEditDialogAbort}
          >
            <Trans>Abort</Trans>
          </Button>
        }
        buttonsRight={
          <Button
            variant="outlined"
            color="secondary"
            onClick={handleSourceEditDialogDone}
          >
      ...
      >
        <Typography>
          <Trans>
            This source cannot be edited while it is in use. To continue, you
            have to disconnect the source.
          </Trans>
        </Typography>
      </Dialog>
      <Dialog
      ...
      >
        <Typography>
          <Trans>
            The deletion of this channel can not be recovered. All publications
            of this channel will be removed.
          </Trans>
        </Typography>
      </Dialog>
