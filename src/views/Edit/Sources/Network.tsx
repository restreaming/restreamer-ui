...
const createInputs = (settings, config, skills) => {
	...
	input.options.push(
		'-thread_queue_size',
		settings.general.thread_queue_size,
	);
	...
	if (settings.general.max_probe_packets !== 2500) {
		input.options.push(
			'-max_probe_packets',
			settings.general.max_probe_packets,
		);
	}
	...
	if (
		...
		settings.general.avoid_negative_ts !== 'auto'
	) {
		input.options.push(
			'-avoid_negative_ts',
			settings.general.avoid_negative_ts,
		);
	}
	...
	if (settings.mode === 'push') {
		...
		input.address = addUsernamePassword(
			input.address,
			settings.username,
			settings.password,
		);
				...
				<AccordionDetails>
					<Grid container spacing={2}>
						...
						{protocolClass === 'http' && (
							<React.Fragment>
								...
								<Grid size={12}>
									...
									<Checkbox
										label={
											<Trans>Force input framerate</Trans>
										}
						...
						<Grid size={12}>
							...
							<Checkbox
								label={
									<Trans>use_wallclock_as_timestamps</Trans>
								}
								checked={
									settings.general.use_wallclock_as_timestamps
								}
								onChange={props.onChange(
									'general',
...
function Pull(props) {
	...
	const supportedProtocol = isSupportedProtocol(
		settings.address,
	...
	return (
		...
		>
            ...
            <Grid size={12}>
				...
				<Typography variant="caption">
					<Trans>
						Supports HTTP (HLS, DASH), RTP, RTSP, RTMP, SRT and
						more.
					</Trans>
				</Typography>
			</Grid>
					...
					{!supportedProtocol ? (
						<Grid sx={{ textAlign: 'center' }} size={12}>
							<BoxText color="dark">
								...
								<Typography>
									<Trans>
										This protocol is unknown or not
										supported by the available FFmpeg
										binary.
									</Trans>
								</Typography>
							</BoxText>
			...
			>
                <Grid sx={{ textAlign: 'center' }} size={12}>
					<BoxText color="dark">
						...
						<Typography>
							<Trans>
								The available FFmpeg binary doesn't support any
