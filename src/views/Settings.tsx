...
const configValues = {
	...
	'storage.disk.cache.enable': {
		tab: 'storage',
		set: (config, value) => {
			config.storage.disk.cache.enable =
				!config.storage.disk.cache.enable;
		},
		unset: (config) => {
	...
	'storage.memory.auth.enable': {
		tab: 'storage',
		set: (config, value) => {
			config.storage.memory.auth.enable =
				!config.storage.memory.auth.enable;
		},
		unset: (config) => {
...
export default function Settings(props) {
	...
	const load = async () => {
		...
		if (data !== null) {
			...
			if (
				config.storage.cors.origins.length === 1 &&
				config.storage.cors.origins[0] === '*'
			) {
				config.storage.cors.allow_all = true;
			}
			config.storage.cors.origins =
				config.storage.cors.origins.join('\n');
			config.storage.disk.cache.types.allow =
				config.storage.disk.cache.types.allow.join('\n');
			config.storage.disk.cache.types.block =
				config.storage.disk.cache.types.block.join('\n');

			config.sessions.ip_ignorelist =
				config.sessions.ip_ignorelist.join('\n');
	...
	const handleSave = async () => {
		...
		if ($config.core.length === 0) {
			...
			config.storage.cors.origins = toArray(
				config.storage.cors.origins,
				'\n',
			);
			...
			config.storage.disk.cache.types.allow = toArray(
				config.storage.disk.cache.types.allow,
				'\n',
			);
			config.storage.disk.cache.types.block = toArray(
				config.storage.disk.cache.types.block,
				'\n',
			);

			config.sessions.ip_ignorelist = toArray(
				config.sessions.ip_ignorelist,
				'\n',
			);

			config.ffmpeg.log.max_lines = toInt(config.ffmpeg.log.max_lines);
			config.ffmpeg.log.max_history = toInt(
				config.ffmpeg.log.max_history,
			);
			config.storage.disk.max_size_mbytes = toInt(
				config.storage.disk.max_size_mbytes,
			);
			config.storage.disk.cache.max_size_mbytes = toInt(
				config.storage.disk.cache.max_size_mbytes,
			);
			config.storage.disk.cache.ttl_seconds = toInt(
				config.storage.disk.cache.ttl_seconds,
			);
			config.storage.disk.cache.max_file_size_mbytes = toInt(
				config.storage.disk.cache.max_file_size_mbytes,
			);
			config.storage.memory.max_size_mbytes = toInt(
				config.storage.memory.max_size_mbytes,
			);
			config.sessions.session_timeout_sec = toInt(
				config.sessions.session_timeout_sec,
			);
			config.sessions.max_bitrate_mbit = toInt(
				config.sessions.max_bitrate_mbit,
			);
			...
			config.rtmp.address_tls = ':' + config.rtmp.address_tls;
			config.rtmp.app = !config.rtmp.app.startsWith('/')
				? '/' + config.rtmp.app
				: config.rtmp.app;
		...
		if (err !== null) {
			if (err.code === 404) {
				notify.Dispatch(
					'error',
					'save:settings',
					i18n._(t`API endpoint not found. Settings not saved.`),
				);
				...
				notify.Dispatch(
					...
					'save:settings',
					i18n._(
						t`There were some errors in the settings. Settings not saved.`,
					),
				);
			} else {
				notify.Dispatch(
					...
					'save:settings',
					i18n._(
						t`There was a problem storing the settings. Settings not saved.`,
					),
				);
			}
			...
			notify.Dispatch(
				...
				'save:settings',
				i18n._(
					t`Settings saved. All changes will be applied after restarting the application.`,
				),
			);
	...
	const handleReload = async () => {
		...
		if (res === false) {
			notify.Dispatch(
				'error',
				'restart',
				i18n._(t`Restarting the application failed.`),
			);
		...
		await props.restreamer.Login(
			$config.data.api.auth.username,
			$config.data.api.auth.password,
		);
	...
	return (
        <Root>
					...
					<TabsVerticalGrid>
						...
						>
							...
							<ErrorTab
								...
								value="network"
								errors={$tabs.network.errors}
							/>
							<ErrorTab
								className="tab"
								label={<Trans>Authorization</Trans>}
								value="auth"
								errors={$tabs.auth.errors}
							/>
							{$expert === true && (
								<ErrorTab
									className="tab"
									label={<Trans>Playback</Trans>}
									value="playback"
									errors={$tabs.playback.errors}
								/>
							)}
							{$expert === true && (
								<ErrorTab
									...
									value="storage"
									errors={$tabs.storage.errors}
								/>
							)}
							<ErrorTab
								className="tab"
								label={<Trans>RTMP</Trans>}
								value="rtmp"
								errors={$tabs.rtmp.errors}
							/>
							<ErrorTab
								className="tab"
								label={<Trans>SRT</Trans>}
								value="srt"
								errors={$tabs.srt.errors}
							/>
							{$expert === true && (
								<ErrorTab
									className="tab"
									label={<Trans>Logging</Trans>}
									value="logging"
									errors={$tabs.logging.errors}
								/>
							)}
						</Tabs>
						<TabPanel
							...
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									<Checkbox
										...
										onChange={handleCheckForUpdates}
									/>
									{$updates.has === true && (
								...
								<Grid size={12}>
									<Checkbox
										label={
											<Trans>
												Send anonymous metrics (helps us
								...
								<Grid size={12}>
									...
									{$config.outdated === true && (
										<Typography variant="caption">
											<Trans>
												The application is using an
												older version of the settings.
											</Trans>
										</Typography>
									)}
								</Grid>
							...
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									<Typography variant="body1">
										...
										<Link
											color="secondary"
											href="https://service.datarhei.com"
											target="_blank"
										>
											<Trans>
								...
								<Grid size={12}>
									...
									<Typography variant="caption">
										<Trans>
											Custom JSON config for datarhei
											Core.
										</Trans>
									</Typography>
								</Grid>
						...
						>
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="host.name"
										messages={$tabs.network.messages}
									/>
									<Typography variant="caption">
										<Trans>
											The public reachable domain name of
											the host this Restreamer is running
											on. Separate multiple domain names
											by a comma.
										</Trans>
									</Typography>
								</Grid>
								...
								<Grid
									...
									/>
									<ErrorBox
										configvalue="tls.address"
										messages={$tabs.network.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Address to listen on for HTTPS
											requests.
										</Trans>
									</Typography>
								</Grid>
								...
								<Grid size={12}>
									<Checkbox
										...
										disabled={
											env('tls.auto') ||
											config.host.name.length === 0
										}
										onChange={handleChange('tls.auto')}
									/>{' '}
									{env('tls.auto') && <Env />}
									<ErrorBox
										configvalue="tls.auto"
										messages={$tabs.network.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Let's Encrypt requires one or more
											public domain names and an
											accessible port 80/TCP.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
									<TextField
										...
										env={env('tls.email')}
										disabled={
											env('tls.email') || !config.tls.auto
										}
										value={config.tls.email}
										onChange={handleChange('tls.email')}
									/>
									<ErrorBox
										configvalue="tls.email"
										messages={$tabs.network.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Please enter your email address to
											signify agreement with the Let's
											Encrypt CA's terms of service and to
											be notified in case of issues.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
								...
								<Grid
									...
									/>
									<ErrorBox
										configvalue="sessions.max_sessions"
										messages={$tabs.network.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Sets a viewer limit for HLS
											sessions. If the limit is exceeded,
											HLS viewers receive the HTTP status
								...
								<Grid
									...
									/>
									<ErrorBox
										configvalue="sessions.max_bitrate_mbit"
										messages={$tabs.network.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Sets a bandwidth limit in Mbit per
											second for outgoing HLS data
											transfer. All services, such as RTMP
											and outgoing processes, are included
											in the calculation. If the bandwidth
											is exceeded, HLS viewers receive the
											HTTP status code 509 (Bandwidth
											Limit Exceeded). 0 is unlimited.
										</Trans>
									</Typography>
								</Grid>
							</Grid>
						...
						<TabPanel value={$tab} index="auth" className="panel">
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									...
									<ErrorBox
										configvalue="api.auth.enable"
										messages={$tabs.auth.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Enabling authorization is strongly
											advised. Otherwise, anybody can
								...
								<Grid size={12}>
									<TextField
										...
										disabled={
											env('api.auth.username') ||
											!config.api.auth.enable
										}
							...
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									<TextField
										...
										disabled={
											env('storage.cors.origins') ||
											config.storage.cors.allow_all
										}
										onChange={handleChange(
											'storage.cors.origins',
										)}
									/>
									<ErrorBox
										configvalue="storage.cors.origins"
										messages={$tabs.playback.messages}
									/>
									<Typography variant="caption">
										<Trans>
											One referrer per line, e.g.
								...
								<Grid size={12}>
									<Checkbox
										label={
											<Trans>
												HLS statistic for the In-memory
												storage
											</Trans>
										}
									...
									<ErrorBox
										configvalue="sessions.enable"
										messages={$tabs.playback.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Allow counting how many viewers the
											stream has.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
									<TextField
										...
										disabled={
											env('sessions.ip_ignorelist') ||
											!config.sessions.enable
										}
									...
									/>
									<ErrorBox
										configvalue="sessions.ip_ignorelist"
										messages={$tabs.playback.messages}
									/>
									<Typography variant="caption">
										<Trans>
											List of IP ranges in CIDR notation,
											e.g., 127.0.0.1/32, that the
											statistics will not record—one IP
											range per line. Leave empty to
											record all sessions.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="sessions.session_timeout_sec"
										messages={$tabs.playback.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Time until an inactive viewer
								...
								<Grid size={12}>
									...
									<ErrorBox
										configvalue="sessions.persist"
										messages={$tabs.playback.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Stores the viewer statistics to the
											disk.
										</Trans>
									</Typography>
								</Grid>
						...
						>
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="storage.memory.auth.enable"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Enabling basic auth is strongly
											advised. Otherwise, anybody could
											write data to /memfs.
										</Trans>
									</Typography>
								</Grid>
								<Grid
									...
									<TextField
										...
										value={
											config.storage.memory.auth.username
										}
										onChange={handleChange(
											'storage.memory.auth.username',
										)}
									/>
									<ErrorBox
										configvalue="storage.memory.auth.username"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
								...
								<Grid
									...
									<Password
										...
										value={
											config.storage.memory.auth.password
										}
										onChange={handleChange(
											'storage.memory.auth.password',
										)}
									/>
									<ErrorBox
										configvalue="storage.memory.auth.password"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
								...
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="storage.memory.max_size_mbytes"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Maximum allowed megabytes of RAM for
											/memfs, 0 for unlimited.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
									<Checkbox
										label={
											<Trans>
												Remove the oldest entries if the
												/memfs is full
											</Trans>
										}
									...
									<ErrorBox
										configvalue="storage.memory.purge"
										messages={$tabs.storage.messages}
									/>
								</Grid>
								<Grid size={12}>
								...
								<Grid size={12}>
									...
									<Typography variant="body1">
										<Trans>
											Settings for /data path. The access
											is protected by{' '}
										</Trans>{' '}
										<Link
											color="secondary"
								...
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="storage.disk.max_size_mbytes"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Maximum allowed megabytes to consume
											from hard disk. 0 for unlimited.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
								...
								<Grid size={12}>
									...
									<ErrorBox
										configvalue="storage.disk.cache.enable"
										messages={$tabs.storage.messages}
									/>
								</Grid>
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="storage.disk.cache.max_size_mbytes"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
											Maximum allowed cache size, 0 for
											unlimited.
										</Trans>
									</Typography>
								</Grid>
								<Grid
									...
									<TextField
										type="number"
										label={
											<Trans>Cache time (Seconds)</Trans>
										}
										env={env(
											'storage.disk.cache.ttl_seconds',
										)}
									...
									/>
									<ErrorBox
										configvalue="storage.disk.cache.ttl_seconds"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
								...
								<Grid
									...
									/>
									<ErrorBox
										configvalue="storage.disk.cache.max_file_size_mbytes"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
								...
								<Grid size={6}>
									<TextField
										...
										value={
											config.storage.disk.cache.types
												.allow
										}
										onChange={handleChange(
											'storage.disk.cache.types.allow',
									...
									/>
									<ErrorBox
										configvalue="storage.disk.cache.types.allow"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
											List of file extensions to cache
											(e.g. ".html"), one per line. Leave
											empty to cache all file types.
										</Trans>
									</Typography>
								</Grid>
								<Grid size={6}>
									...
									/>
									<ErrorBox
										configvalue="storage.disk.cache.types.block"
										messages={$tabs.storage.messages}
									/>
									<Typography variant="caption">
										<Trans>
											List of file extensions not to cache
											(e.g. ".m3u8"), one per line. Leave
											empty for none.
										</Trans>
									</Typography>
								</Grid>
						...
						<TabPanel value={$tab} index="rtmp" className="panel">
							<Grid container spacing={2}>
								...
								<Grid size={12}>
									...
									/>{' '}
									{env('rtmp.enable') && (
										<Env style={{ marginRight: '2em' }} />
									)}
									<ErrorBox
										configvalue="rtmp.enable"
										messages={$tabs.rtmp.messages}
									/>
									<Checkbox
								...
								<Grid size={12}>
									...
									/>
									<ErrorBox
										configvalue="rtmp.token"
										messages={$tabs.rtmp.messages}
									/>
									<Typography variant="caption">
										<Trans>
											RTMP token for publishing and
											playing. The token is the value of
											the URL query parameter 'token.'
										</Trans>
									</Typography>
								</Grid>
							</Grid>
						...
						<TabPanel value={$tab} index="srt" className="panel">
							<Grid container spacing={2}>
								...
								<Grid
									...
									/>
									<ErrorBox
										configvalue="srt.token"
										messages={$tabs.srt.messages}
									/>
									<Typography variant="caption">
										<Trans>
											SRT token for publishing and
											playing. The token is the value of
											the streamid parameter 'token.'
										</Trans>
									</Typography>
								</Grid>
								<Grid size={12}>
									<Password
										...
										error={
											config.srt.passphrase &&
											config.srt.passphrase.length < 10
										}
										helperText={
											config.srt.passphrase &&
											config.srt.passphrase.length <
												10 ? (
												<Trans>
													Passphrase must be between
													10 and 79 characters long
												</Trans>
											) : (
												false
											)
						...
						>
							<Grid container spacing={2}>
								...
								<Grid size={6}>
									...
									<ErrorBox
										configvalue="log.level"
										messages={$tabs.logging.messages}
									/>
									<Typography variant="caption">
								...
								<Grid size={6}>
									...
									/>
									<ErrorBox
										configvalue="ffmpeg.log.max_lines"
										messages={$tabs.logging.messages}
									/>
									<Typography variant="caption">
										<Trans>
								...
								<Grid size={6}>
									<TextField
										type="number"
										label={
											<Trans>Maximum log histroy</Trans>
										}
									...
									/>
									<ErrorBox
										configvalue="ffmpeg.log.max_history"
										messages={$tabs.logging.messages}
									/>
									<Typography variant="caption">
										<Trans>
					...
					<PaperContent>
						{$restart.timeout === false ? (
							...
							<React.Fragment>
								<Typography variant="body1">
									<Trans>
										Reconnecting to Restreamer Core failed
										for the last {RETRIES} seconds.
									</Trans>
								</Typography>
								<Typography variant="body1" sx={{ mt: '1em' }}>
									<Trans>
										If you enabled Let's Encrypt TLS it
										might take some time to acquire the
										certificates. Make sure that Restreamer
										Core is reachable via port 80 from the
										internet. Please check the console log
										of Restreamer Core.
									</Trans>
								</Typography>
								<Typography variant="body1" sx={{ mt: '1em' }}>
									<Trans>
										If you changed the ports, it might be
										that Restreamer Core restarted already,
										but it is now available on a different
										port.
									</Trans>
								</Typography>
							</React.Fragment>
			...
			>
				<Typography variant="body1">
					<Trans>
						You have changed the configuration. In order for the
						changes to take effect, you have to restart the
						application. Do you want to restart now?
					</Trans>
				</Typography>
			</Dialog>
            <Dialog
