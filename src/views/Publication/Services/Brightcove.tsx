...
const description = (
	<Trans>
		Transmit the main source to a Brightcove Live Service. More details
		about the settings can be found{' '}
		<Link
			color="secondary"
			target="_blank"
			href="https://live.support.brightcove.com/get-started/brightcove-live-training-videos.html"
		>
			here
		</Link>
		.
...
function Service(props) {
	...
	const createOutput = (settings) => {
		const output = {
			address:
				settings.protocol + settings.address + settings.stream_name,
			options: ['-f', 'flv'],
		};
