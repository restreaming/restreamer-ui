...
const version = '2.0';
const stream_key_link_rtmp =
	'https://docs.datarhei.com/restreamer/knowledge-base/manual/system-settings/rtmp';
const stream_key_link_srt =
	'https://docs.datarhei.com/restreamer/knowledge-base/manual/system-settings/srt';
const description = (
	<Trans>
		Transmit the main source to an datarhei Core Ressource. More details
			...
			if (settings.protocol === 'srt') {
				settings.v2_address =
					...
					(settings.token.length !== 0
						...
						: '') +
					(settings.srt_passphrase.length !== 0
						? `&passphrase=${settings.srt_passphrase}`
						: '');
				settings.v2_stream_id =
					`#!:m=publish,r=${settings.stream_name}` +
					(settings.token.length !== 0
						? `,token=${settings.token}`
						: '');
			...
			if (url.protocol === 'srt:') {
				if (url.hash) {
					if (url.hash.includes('passphrase=')) {
						...
						settings.v2_passphrase = url.hash
							.split('&passphrase=')[1]
							.split('&')[0];
					} else {
						settings.v2_stream_id = url.hash.split('&')[0];
					...
					if (url.query.includes('passphrase=')) {
						settings.v2_stream_id = url.query
							.split('streamid=')[1]
							.split('&')[0];
						settings.v2_passphrase = url.query
							.split('&passphrase=')[1]
							.split('&')[0];
					} else {
				...
				<BoxText alignItems="flex-start" justifyContent="flex-start">
					{!settings.v2_address && (
						<Typography>
							...
							<br />
							<Trans>
								1. Switch to the interface of the target
								Restreamer.
							</Trans>
							<br />
							<Trans>
								2. Create a new channel and select RTMP or SRT
								server.
							</Trans>
							<br />
							<Trans>
								3. Copy the URL and paste it in the "Target
								address" field.
							</Trans>
						</Typography>
					)}
