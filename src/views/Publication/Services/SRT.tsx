...
const description = (
	<Trans>
		Transmit the main source to an SRT Server. More details about the
		settings can be found{' '}
		<Link
			color="secondary"
			target="_blank"
			href="https://ffmpeg.org/ffmpeg-all.html#srt"
		>
			here
		</Link>
		.
...
function Service(props) {
	...
	const handleChange = (what) => (event) => {
		...
		if (what in settings.params) {
			if (
				['enforced_encryption', 'tlpktdrop', 'nakreport'].includes(what)
			) {
	...
	const createOutput = (settings) => {
		...
		const output = {
			address:
				settings.protocol + settings.address + '?' + params.join('&'),
			options: options,
		};
        ...
        <Grid container spacing={2}>
            ...
            <Grid size={12}>
				<TextField
					...
					fullWidth
					label="streamid"
					value={settings.params.streamid}
						...
						<Grid container spacing={2}>
							...
							<Grid
								...
								<Select
									...
									onChange={handleChange('smoother')}
								>
							...
							<Grid
								...
								<Select
									...
									onChange={handleChange('transtype')}
								>
									<MenuItem value="live">live</MenuItem>
									<MenuItem value="file">file</MenuItem>
								</Select>
							...
							<Grid size={12}>
								<Checkbox
									...
									onChange={handleChange('nakreport')}
								/>
								<Checkbox
									label="tlpktdrop"
									checked={settings.params.tlpktdrop}
							...
							<Grid
								...
								<Select
									...
									onChange={handleChange('pbkeylen')}
								>
							...
							<Grid size={12}>
								<Checkbox
									...
									checked={
										settings.params.enforced_encryption
									}
