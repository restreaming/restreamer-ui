...
const description = (
	<Trans>
		Transmit snapshots of the main source to an HTTP/S Server. More details
		about the settings can be found{' '}
		<Link
			color="secondary"
			target="_blank"
			href="https://ffmpeg.org/ffmpeg-all.html#image2"
		>
			here
		</Link>
		.
					...
					<AccordionDetails>
						<Grid container spacing={2}>
							...
							<Grid item xs={12}>
								<Checkbox
									label="frame_pts"
									checked={settings.options.frame_pts}
									onChange={handleChange('frame_pts')}
								/>
								<Checkbox
									label="update"
									checked={settings.options.update}
									onChange={handleChange('update')}
								/>
								<Checkbox
									label="strftime"
									checked={settings.options.strftime}
									onChange={handleChange('strftime')}
								/>
							</Grid>
							<Grid item xs={12}>
								<Divider />
