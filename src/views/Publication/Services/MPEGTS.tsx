...
const description = (
	<Trans>
		Transmit the main source to an MPEG-TS Service. More details about the
		settings can be found here{' '}
		<Link
			color="secondary"
			target="_blank"
			href="https://ffmpeg.org/ffmpeg-all.html#mpegts-1"
		>
			here
		</Link>
		.
	...
	const createOutput = (settings) => {
		...
		for (let key in settings.options) {
			...
			if (['service_provider', 'service_name'].includes(key)) {
				options.push(
					'-metadata',
					key + '="' + String(settings.options[key]) + '"',
				);
			} else if (
				key === 'mpegts_service_type' &&
				settings.options.mpegts_service_type === 'hex_value'
			) {
				if (
					settings.options.mpegts_service_type_hex_value.length !== 0
				) {
					options.push(
						'-mpegts_service_type',
						String(settings.options.mpegts_service_type_hex_value),
					);
				}
					...
					<AccordionDetails>
						<Grid container spacing={2}>
							...
							<Grid item xs={12} md={6}>
								...
								>
									<MenuItem value="hex_value">
										hex_value
									</MenuItem>
									<MenuItem value="digital_tv">
										digital_tv
									</MenuItem>
									<MenuItem value="digital_radio">
										digital_radio
									</MenuItem>
									<MenuItem value="teletext">
										teletext
									</MenuItem>
									<MenuItem value="advanced_codec_digital_radio">
										advanced_codec_digital_radio
									</MenuItem>
									<MenuItem value="mpeg2_digital_hdtv">
										mpeg2_digital_hdtv
									</MenuItem>
									<MenuItem value="advanced_codec_digital_sdtv">
										advanced_codec_digital_sdtv
									</MenuItem>
									<MenuItem value="advanced_codec_digital_hdtv">
							...
							<Grid item xs={12} md={6}>
								<TextField
									...
									value={
										settings.options.mpegts_pmt_start_pid
									}
									onChange={handleChange(
							...
							<Grid item xs={12}>
								<Checkbox
									...
									onChange={handleChange('mpegts_copyts')}
								/>
								<Checkbox
									label="omit_video_pes_length"
									checked={
										settings.options.omit_video_pes_length
									}
									onChange={handleChange(
										'omit_video_pes_length',
									)}
