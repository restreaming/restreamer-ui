...
const description = (
	<Trans>
		Transmits your video as an RTMP stream with the required key generated
		in YouTube Studio. You can find more information on setting up a live
		stream at YouTube's{' '}
		<Link
			color="secondary"
			target="_blank"
			href="https://creatoracademy.youtube.com/"
		>
			Creator Academy
		</Link>
		.
...
const image_copyright = (
	<Trans>
		...
		<Link
			color="secondary"
			target="_blank"
			href="https://www.youtube.com/howyoutubeworks/policies/copyright/#support-and-troubleshooting"
		>
			here
		</Link>
		.
	...
	const createOutput = (settings) => {
		...
		if (settings.mode === 'rtmps') {
			...
			if (settings.backup === true) {
				outputs.push({
					address:
						'rtmps://b.rtmp.youtube.com/live2?backup=1/' +
						settings.stream_key,
					options: options.slice(),
				});
			}
	...
	return (
		<Grid container spacing={2}>
			<Grid item xs={12}>
				...
				>
					{allowRTMPS === true && (
						<MenuItem value="rtmps">RTMP</MenuItem>
					)}
			...
			<Grid item xs={12}>
				...
				/>
				<Checkbox
					label={<Trans>Backup stream</Trans>}
