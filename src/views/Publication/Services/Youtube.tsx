...
const description = (
  <Trans>
    Transmits your video as an RTMP stream with the required key generated in
    YouTube Studio. You can find more information on setting up a live stream at
    YouTube's{" "}
    <Link
      color="secondary"
      target="_blank"
  ...
  const createOutput = (settings: Any) => {
    ...
    if (settings.mode === "rtmps") {
      ...
      if (settings.backup === true) {
        outputs.push({
          address:
            "rtmps://b.rtmp.youtube.com/live2?backup=1/" + settings.stream_key,
          options: options.slice(),
        });
      }
