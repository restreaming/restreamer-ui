...
const description = (
  <Trans>
    Transmits your video stream with the required key, which was generated in
    Twitter Producer. You can find more information on seting up a live stream
    at Twitter's{" "}
    <Link
      color="secondary"
      target="_blank"
      href="https://help.twitter.com/en/using-twitter/how-to-use-live-producer"
    >
      Producer
    </Link>
    .
...
function Service(props: Any) {
  ...
  const createOutput = (settings: Any) => {
    ...
    if (settings.mode === "rtmps") {
      ...
      const base =
        "https://" + settings.region + ".pscp.tv:443/x/" + settings.stream_key;
      outputs.push({
