...
export default function Edit(props) {
	...
	const mount = async () => {
		...
		const proc = await props.restreamer.GetIngest(channelid, [
			'state',
			'metadata',
		]);
		if (proc === null) {
			notify.Dispatch(
				'warning',
				'notfound:ingest',
				i18n._(t`Main channel not found`),
			);
	...
	const handleLogoUpload = async (data, extension) => {
		const path = await props.restreamer.UploadLogo(
			_channelid,
			data,
			extension,
		);
	...
	const handleDone = async () => {
		...
		notify.Dispatch(
			'success',
			'save:player',
			i18n._(t`Player settings saved`),
		);
	};
	...
	const poster = $settings.poster
		...
		: props.restreamer.GetChannelAddress('snapshot+' + storage, _channelid);
	const playerAddress = props.restreamer.GetPublicAddress(
		'player',
		_channelid,
	);
							...
							<Grid container spacing={2}>
								<Grid item xs={12} md={6}>
									<TextField
										...
										id="ga-id"
										label={
											<Trans>Google Analytics ID</Trans>
										}
						...
						<TabPanel value={$tab} index="playback">
							<Grid container spacing={2}>
								<Grid item xs={12}>
									...
									/>
									<Checkbox
										label={<Trans>Mute</Trans>}
										checked={$settings.mute}
										onChange={handleChange('mute')}
									/>
									<Checkbox
