...
export default function Incompatible(props: any) {
  const text =
    props.type === "core" ? (
      <Trans>
        This version of the UI doesn't support the connected Core ({props.have}
        ). The UI requires {props.want}. Please use a compatible version of the
        UI.
      </Trans>
    ) : props.type === "ffmpeg" ? (
      <Trans>
        This version of the UI doesn't support the available FFmpeg binary (
        {props.have}). The UI requires {props.want}. Please use a supported
        FFmpeg binary.
      </Trans>
    ) : (
