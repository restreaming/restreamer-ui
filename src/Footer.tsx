...
function Resources(props) {
  ...
  const update = async () => {
    ...
    resources.system.mem_used =
      (resources.system.mem_used_bytes / resources.system.mem_total_bytes) *
      100;

    resources.core.disk_used = -1;
    if (resources.core.disk_limit_bytes > 0) {
      resources.core.disk_used =
        (resources.core.disk_used_bytes / resources.core.disk_limit_bytes) *
        100;
    }
    ...
    if (resources.core.memfs_limit_bytes > 0) {
      resources.core.memfs_used =
        (resources.core.memfs_used_bytes / resources.core.memfs_limit_bytes) *
        100;
    }
    ...
    if (resources.core.net_limit_kbit > 0) {
      resources.core.net_used =
        (resources.core.net_used_kbit / resources.core.net_limit_kbit) * 100;
    }

    resources.core.sessions = -1;
    if (resources.core.session_limit > 0) {
      resources.core.sessions =
        (resources.core.session_used / resources.core.session_limit) * 100;
    }
        ...
        >
          ...
          >
            <ListItemText
              ...
              secondary={
                <Typography variant="body2">
                  <Number value={system.mem_used_bytes / 1024 / 1024} /> /{" "}
                  <Number value={system.mem_total_bytes / 1024 / 1024} />{" "}
                  <Trans>MB</Trans>
                </Typography>
              }
            />
          ...
          <ListItem divider>
            {core.sessions >= 0 ? (
              <ListItemText
                ...
                secondary={
                  <Typography variant="body2">
                    <Number value={core.session_used} /> /{" "}
                    <Number value={core.session_limit} /> <Trans>Viewer</Trans>
                  </Typography>
                }
              />
            ) : (
          ...
          <ListItem divider>
            {core.net_used >= 0 ? (
              <ListItemText
                ...
                secondary={
                  <Typography variant="body2">
                    <Number value={core.net_used_kbit / 1024} /> /{" "}
                    <Number value={core.net_limit_kbit / 1024} /> Mbit/s
                  </Typography>
                }
              />
            ) : (
          ...
          <ListItem divider>
            {core.memfs_used >= 0 ? (
              <ListItemText
                primary={
                  <Typography variant="body3">
                    {core.memfs_used.toFixed(0)}%{" "}
                    <Trans>In-memory storage</Trans>
                  </Typography>
                }
                secondary={
                  <Typography variant="body2">
                    <Number value={core.memfs_used_bytes / 1024 / 1024} /> /{" "}
                    <Number value={core.memfs_limit_bytes / 1024 / 1024} /> MB
                  </Typography>
                }
              />
            ) : (
          ...
          <ListItem>
            {core.disk_used >= 0 ? (
              <ListItemText
                ...
                secondary={
                  <Typography variant="body2">
                    <Number value={core.disk_used_bytes / 1024 / 1024} /> /{" "}
                    <Number value={core.disk_limit_bytes / 1024 / 1024} /> MB
                  </Typography>
                }
              />
            ) : (
