...
const getDefaultMetadata = (...args: Any[]) => {
  void args;
  // poor mans deep copy
  return JSON.parse(JSON.stringify(defaultMetadata));
};

const getDefaultIngestMetadata = (...args: Any[]) => {
  void args;
  // poor mans deep copy
  return JSON.parse(JSON.stringify(defaultIngestMetadata));
};

const getDefaultEgressMetadata = (...args: Any[]) => {
  void args;
  // poor mans deep copy
...
const mergeMetadata = (
  ...
  base: DynamicObject = getDefaultMetadata(),
) => {
  if (!metadata) {
    metadata = {};
  }
...
const ingestTransformers = {
  "1.2.0": (metadata: Any) => {
    for (let p = 0; p < metadata.profiles.length; p++) {
      ...
      profile.audio.filter.settings.aresample.graph = Filters.Audio.Get(
        "aresample",
      ).createGraph(profile.audio.filter.settings.aresample.settings);
      profile.audio.filter.graph =
        profile.audio.filter.settings.aresample.graph;
    }
...
const mergeIngestMetadata = (
  metadata: DynamicObject,
  base: DynamicObject = getDefaultIngestMetadata(),
) => {
  if (!metadata) {
    metadata = {};
  }
  ...
  if (!Array.isArray(metadata.sources)) {
    ...
    for (let i = 0; i < metadata.sources.length; i++) {
      metadata.sources[i] = initSource(
        metadata.sources[i].type,
        metadata.sources[i],
      );
    }
  }
...
const mergeEgressMetadata = (
  metadata: DynamicObject,
  base: DynamicObject = getDefaultEgressMetadata(),
) => {
  if (!metadata) {
    metadata = {};
  }
...
const validateProfile = (
  ...
  if (profile.video.source !== -1 && profile.video.source < sources.length) {
    ...
    if (
      ...
      profile.video.stream < source.streams.length
    ) {
  ...
  if (profile.audio.source !== -1 && profile.audio.source < sources.length) {
    ...
    if (
      ...
      profile.audio.stream < source.streams.length
    ) {
  ...
  if (requireVideo === true) {
    if (
      profile.video.encoder.coder === "none" ||
      profile.video.source === -1 ||
      profile.video.stream === -1
    ) {
      complete = false;
...
const createInputsOutputs = (
  ...
  for (const profile of profiles) {
    ...
    if (
      profile.video.encoder.coder !== "copy" &&
      (profile.video.filter.graph.length !== 0 ||
        profile.video.encoder.mapping.filter.length !== 0)
    ) {
    ...
    if (
      ...
      input.options = [
        ...profile.audio.decoder.mapping.local,
        ...input.options,
      ];
      ...
      if (
        ...
        (profile.audio.filter.graph.length !== 0 ||
          profile.audio.encoder.mapping.filter.length !== 0)
      ) {
...
const preselectProfile = (
  ...
  const isAudioPlausible = (streams: Any, audio: Any) => {
    ...
    if (
      streams[audio.stream].codec !== "aac" &&
      streams[audio.stream].codec !== "mp3"
    ) {
    ...
    if (isVideoPlausible(streams, profile.video) === false) {
      ...
      for (let i = 0; i < streams.length; i++) {
        ...
        if (streams[i].codec === "h264") {
          ...
          const coder = Coders.Video.GetCoderForCodec("h264", encoders.video);
          if (coder === null) {
            video.encoder.coder = "none";
          } else {
