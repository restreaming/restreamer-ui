...
class Restreamer {
	...
	async Login(username, password) {
		...
		const [data, err] = await this._call(
			this.api.Login,
			username,
			password,
		);
		if (err !== null) {
			this._dispatchEvent(
				'error',
				'login',
				i18n._(t`Login failed: ${err.message}`),
			);
			return false;
		}
		...
		if (about === null) {
			this._dispatchEvent(
				'error',
				'login',
				i18n._(t`Login failed: Couldn't load API details`),
			);
			return false;
		}
	...
	Compatibility() {
		...
		compatibility.core.compatible = SemverSatisfies(
			compatibility.core.have,
			compatibility.core.want,
		);
		compatibility.ffmpeg.compatible = SemverSatisfies(
			compatibility.ffmpeg.have,
			compatibility.ffmpeg.want,
		);

		if (
			compatibility.core.compatible === true &&
			compatibility.ffmpeg.compatible === true
		) {
	...
	async RefreshToken() {
		...
		if (err !== null) {
			this._dispatchEvent(
				'error',
				'auth',
				i18n._(t`Failed to refresh token: ${err.message}`),
			);
			return false;
		}
	...
	async _initConfig() {
		...
		if (
			(config.http.secure && port !== '443') ||
			(!config.http.secure && port !== '80')
		) {
		...
		config.source.network.rtmp.token = encodeURIComponent(
			val.config.rtmp.token,
		);
		...
		config.source.network.rtmp.local =
			rtmp_host.length !== 0 ? rtmp_host : 'localhost';
		if (rtmp_port !== '1935') {
		...
		if (config.source.network.rtmp.secure === true) {
			...
			if (rtmp_port !== '1935') {
				config.source.network.rtmp.host =
					config.hostname + ':' + rtmp_port;
			}
		}
		...
		if (config.memfs.auth.enable === true) {
			config.source.network.hls.credentials =
				encodeURIComponent(config.memfs.auth.username) +
				':' +
				encodeURIComponent(config.memfs.auth.password);
		}
	...
	GetPublicHTTPAddress() {
		...
		const address =
			(config.http.secure === true ? 'https://' : 'http://') +
			config.http.host;
	...
	GetPublicAddress(what, channelid) {
		...
		if (what === 'rtmp') {
			...
			address =
				(config.http.secure === true ? 'https://' : 'http://') +
				`${host}${port}/` +
				this.GetChannelPosterPath(channelid, 'memfs');
			...
			address =
				(config.http.secure === true ? 'https://' : 'http://') +
				`${host}${port}/` +
				this.GetChannelPosterPath(channelid, 'diskfs');
			...
			address =
				(config.http.secure === true ? 'https://' : 'http://') +
				`${host}${port}/` +
				this.GetChannelManifestPath(channelid, 'memfs');
			...
			address =
				(config.http.secure === true ? 'https://' : 'http://') +
				`${host}${port}/` +
				this.GetChannelManifestPath(channelid, 'diskfs');
		} else if (what === 'player') {
			// player
			address =
				(config.http.secure === true ? 'https://' : 'http://') +
				`${config.http.host}/` +
				this.GetChannelPlayerPath(channelid);
		}
	...
	async _discoverChannels() {
		...
		const reIngest = new RegExp(
			'^restreamer-ui:ingest:([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$',
		);
		const reEgress = new RegExp(
			'^restreamer-ui:egress:([0-9a-z]+):([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$',
		);
	...
	ListChannels() {
		...
		for (const channel of this.channels.values()) {
			channels.push({
				...
				thumbnail: this.GetChannelAddress(
					'snapshot+memfs',
					channel.channelid,
				),
	...
	async UpsertIngest(channelid, global, inputs, outputs, control) {
		...
		let segmentFilePath =
			`${channel.channelid}` +
			(control.hls.master_playlist ? `_{outputid}_%04d` : '_%04d');
		if (hlsStorage === 'diskfs') {
			// diskfs (path structure)
			segmentFilePath =
				`${channel.channelid}` +
				(control.hls.master_playlist
					? `/{outputid}/%Y%m%d/%s`
					: '/%Y%m%d/%s');
		}
		...
		const hls_segment_filename =
			...
			(tee_muxer ? '^:' : '') +
			`}/${segmentFilePath}.` +
			(!control.hls.lhls && control.hls.version === 7 ? 'mp4' : 'ts');
				...
				switch (version) {
					case 6:
						return [
							...
							[
								'hls_flags',
								'append_list+delete_segments+program_date_time+independent_segments+temp_file',
							],
					...
					case 7:
						// fix Malformed AAC bitstream detected for hls version 7
						if (
							output.options.includes('-codec:a') &&
							output.options.includes('copy')
						) {
							if (!tee_muxer) {
		...
		const hls_params_raw = getHLSParams(
			control.hls.lhls,
			control.hls.version,
		);
		...
		if (control.hls.master_playlist) {
			hls_params_raw.push(
				...
				['master_pl_publish_rate', `${control.hls.segmentDuration}`],
			);
		}
		...
		if (tee_muxer) {
			...
			output.address =
				...
				(rtmp_enabled
					? `|[f=flv]{rtmp,name=${channel.channelid}.stream}`
					: '') +
				(srt_enabled
					? `|[f=mpegts]{srt,name=${channel.channelid},mode=publish}`
					: '');
		} else {
			// ['-f', 'hls', '-start_number', '0', ...]
		...
		let cleanupSegmentFilePath =
			`${channel.channelid}` +
			(control.hls.master_playlist ? `_{outputid}_**` : '_**');
		if (hlsStorage === 'diskfs') {
			// diskfs (path structure)
			cleanupSegmentFilePath =
				`${channel.channelid}` +
		...
		const cleanup_hls_segment_filename =
			`${hlsStorage}:/${cleanupSegmentFilePath}.` +
			(!control.hls.lhls && control.hls.version === 7 ? 'mp4' : 'ts');
		...
		output.cleanup.push(
			...
			},
			{
				...
				max_file_age_seconds: control.hls.cleanup
					? parseInt(control.hls.segmentDuration) *
						(parseInt(control.hls.listSize) + 6)
					: 0,
				purge_on_delete: true,
			},
			{
	...
	async UpdatePlayer(channelid) {
		...
		const templateData = {
			...
			poster: this.GetChannelPosterPath(
				channelid,
				metadata.control.hls.storage,
			),
		...
		if (metadata.player.poster.length !== 0) {
			...
			templateData.poster_url = this.PrefixPublicHTTPAddress(
				metadata.player.poster,
			);
		}

		// upload player.html
		let player = await this._getLocalAssetAsString(
			`/_player/${playerType}/player.html`,
		);
		player = Handlebars.compile(player)(templateData);
		await this._uploadAssetData(`/${channelid}.html`, player);

		// upload oembed.json
		let embed = await this._getLocalAssetAsString(
			'/_player/oembed.json.in',
		);
		...
		await this._uploadAssetData(
			`/channels/${channelid}/oembed.json`,
			embed,
		);
	...
	async UpdatePlayerConfig(channelid, metadata) {
		...
		const playerConfig = {
			...metadata.player,
			source: this.GetChannelManifestPath(
				channelid,
				metadata.control.hls.storage,
			),
			poster: this.GetChannelPosterPath(
		...
		await this._uploadAssetData(
			`/channels/${channelid}/config.js`,
			'var playerConfig = ' + JSON.stringify(playerConfig),
		);
	}
	...
	async UpdatePlayersite() {
		...
		for (const item of channels) {
			...
			const templateData = {
				...
				channel_description_html:
					ingestMetadata.meta.description.replace(
						/(?:\r\n|\r|\n)/g,
						'<br />',
					),
				channel_creator_name: ingestMetadata.meta.author.name,
				channel_creator_description:
					ingestMetadata.meta.author.description,
				channel_creator_description_html:
					ingestMetadata.meta.author.description.replace(
						/(?:\r\n|\r|\n)/g,
						'<br />',
					),
				channel_license: ingestMetadata.license,
				channel_poster: this.GetChannelPosterPath(
					item.channelid,
					ingestMetadata.control.hls.storage,
				),
				channel_width: 640,
				channel_height: 360,
			};
			...
			if (settings.template !== '!default') {
				playersite = await this.GetPlayersiteTemplate(
					settings.template,
				);
			...
			if (settings.template === '!default') {
				playersite = await this._getLocalAssetAsString(
					'/_playersite/index.html',
				);
			}
			...
			playersite = Handlebars.compile(playersite)(templateData);
			await this._uploadAssetData(
				'/playersite_' + item.channelid + '.html',
				playersite,
			);
		...
		await this._uploadAssetData(
			'/playersite/player.js',
			await this._getLocalAssetAsString(
				`/_playersite/${settings.player}.js`,
			),
		);
		...
		(async () => {
			...
			try {
				response = await fetch(
					'https://service.datarhei.com/api/v1/app_version',
					{
						method: 'PUT',
						headers: {
	...
	async _getProcessDebug(id) {
		...
		if (p.config) {
			...
			for (const i in p.config.output) {
				...
				p.config.output[i].options =
					p.config.output[i].options.map(anonymize);
			}
		}
	...
	async _getConfigDebug() {
		...
		config.api.auth.auth0.tenants = config.host.name.map(
			(e) => '[anonymized]',
		);
	...
	_getProgressFromState(state) {
		...
		if (state.exec === 'starting') {
			...
			progress.reconnect =
				state.reconnect_seconds === undefined
					? -1
		...
		if (progress.state === 'connected') {
			...
			progress.q =
				state.progress.quality === undefined
					? -1
					: state.progress.quality;
	...
	async _getResources() {
		...
		const resources = {
			...
			system: {
				...
				mem_used_bytes:
					getValue(metrics, 'mem_total') -
					getValue(metrics, 'mem_free'),
				mem_total_bytes: getValue(metrics, 'mem_total'),
			},
			core: {
				session_used: getValue(metrics, 'session_active', {
					collector: 'hls',
				}),
				session_limit: getValue(metrics, 'session_limit', {
					collector: 'hls',
				}),
				net_used_kbit:
					getValue(metrics, 'session_txbitrate', {
						collector: 'hls',
					}) / 1024,
				net_limit_kbit:
					getValue(metrics, 'session_maxtxbitrate', {
						collector: 'hls',
					}) / 1024,
				memfs_used_bytes: getValue(metrics, 'filesystem_usage', {
					name: 'memfs',
				}),
				memfs_limit_bytes: getValue(metrics, 'filesystem_limit', {
					name: 'memfs',
				}),
				disk_used_bytes: getValue(metrics, 'filesystem_usage', {
					name: 'diskfs',
				}),
				disk_limit_bytes: getValue(metrics, 'filesystem_limit', {
					name: 'diskfs',
				}),
			},
		};
