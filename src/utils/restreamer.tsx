...
class Restreamer {
  ...
  async _initSkills() {
    ...
    for (const device of val.devices.demuxers) {
      if (
        !["avfoundation", "video4linux2", "alsa", "fbdev"].includes(device.id)
      ) {
        continue;
      }
  ...
  GetPublicHTTPAddress() {
    ...
    const address =
      (config.http.secure === true ? "https://" : "http://") + config.http.host;
  ...
  async UpsertIngest(channelid: any, global: any, inputs: any, outputs: any, control: any) {
    ...
    if (hlsStorage === "diskfs") {
      ...
      segmentFilePath =
        `${channel.channelid}` +
        (control.hls.master_playlist ? `/{outputid}/%Y%m%d/%s` : "/%Y%m%d/%s");
    }
  ...
  async UpdatePlayersite() {
    ...
    await this._uploadAssetData(
      ...
      await this._getLocalAssetAsString(`/_playersite/${settings.player}.js`),
    );
  ...
  async ListProcesses(filter: string[] = [], ids: string[] = []) {
  ...
  _getProgressFromState(state: any) {
    ...
    if (state.exec === "starting") {
      ...
      progress.reconnect =
        state.reconnect_seconds === undefined ? -1 : state.reconnect_seconds;
    ...
    if (progress.state === "connected") {
      ...
      progress.q =
        state.progress.quality === undefined ? -1 : state.progress.quality;
  ...
  async _getResources() {
    ...
    const resources = {
      ...
      system: {
        ...
        mem_used_bytes:
          getValue(metrics, "mem_total") - getValue(metrics, "mem_free"),
